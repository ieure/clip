...
** Packages
   ...
   and =FORMAT= (which both come from the =COMMON-LISP= package[fn:5]).
...
** Defining the System
   ...
   someone wanted to reuse the core =GREET= code in their own program[fn:4],
...
[fn:4] For example, to have their program greet the user when it
starts, or to create a massively multiplayer online Hello World
(MMOHW).

[fn:5] =:CL= is an alias for the =:COMMON-LISP= package, so =(:use

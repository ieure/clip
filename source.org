...
** Packages
   ...
   If you wanted to refer to symbols in other packages, you’d add them
   to that list:
   ...
   both loads /and/ refers, Lisp doesn’t work this way; you have to
   ensure that =:foo= and =:bar= have been loaded already.
...
** Building an Image
   ...
   concerns for anything more complex than one-off scripts[fn:8].  Its
...
** Quicklisp
   ...
   or rbenv.  There’s no [[https://res.cloudinary.com/practicaldev/image/fetch/s--asKSgsVB--/c_imagga_scale,f_auto,fl_progressive,h_420,q_auto,w_1000/https://thepracticaldev.s3.amazonaws.com/i/9i6bs4g6cx05jeagfhum.png][=node_modules=]].
...
** The New =MAIN=
   ...
   puts its exported symbols into the current package, but exported
   symbols from any loaded package can always be referred to by
   prefixing them with the package name and a colon,
   ex. =unix-opts:get-opts=.
...
[fn:6] You can still refer to them by qualifying with the package:
=(common-lisp:defun () …)=
